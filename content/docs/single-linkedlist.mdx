...
```
Single Linked List dengan 4 node:
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
...
### Latihan 3: Pencarian dan Penghapusan Data
**Objektif**: Sistem pencarian dan penghapusan data berdasarkan nilai.

**Spesifikasi:**
- Cari data dalam list
- Hapus data jika ditemukan
- Berikan informasi jika data tidak ditemukan

### Latihan 4: Operasi List Lanjutan
**Objektif**: Implementasi operasi tambahan pada Single Linked List.

**Spesifikasi:** 
- Hitung jumlah elemen dalam list
- Cari nilai maksimum dan minimum
- Reverse list (balik urutan)
...
- **Memory Management**: Selalu lakukan dealokasi untuk menghindari memory leak.
- **Null Pointer**: Selalu cek apakah pointer tidak NULL sebelum mengakses.
- **Traversal**: Gunakan loop while dengan kondisi temp != NULL.
- **Edge Cases**: Perhatikan kasus list kosong dan list dengan satu elemen.
- **Complexity**: Insertion/Deletion O(1), Search/Access O(n).
